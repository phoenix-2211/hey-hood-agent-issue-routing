...
api = FastAPI(
    title="Hey Hood — Issue Routing Agent",
    description="ADK 2.0 agent for assigning issues to ward officials",
    version="1.0.0"
)
...
class IssueRoutingInput(BaseModel):
    issue_id: str
    title: str
    category: str
    severity: str
...
@api.get("/health")
async def health():
    return {"status": "healthy", "agent": "issue_routing"}

@api.post("/run", response_model=AgentResponse)
async def run_agent(input_data: IssueRoutingInput):
    try:
